...
from models.SelfCBR import SelfCBR
...
def get_cmd():
    ...
    parser.add_argument("-d", "--dataset", default="iFashion", type=str, help="which dataset to use, options: NetEase, Youshu, iFashion")
    parser.add_argument("-m", "--model", default="SelfCBR", type=str, help="which model to use, options: SelfCBR")
...
def main():
    ...
    assert paras["model"] in ["SelfCBR"], "Pls select models from: SelfCBR"
    ...
    for lr, l2_reg, item_level_dropout, bundle_level_dropout, bundle_agg_dropout, embedding_size, num_layers, c_lambda, c_temp in \
            product(conf['lrs'], conf['l2_regs'], conf['item_level_dropouts'], conf['bundle_level_dropouts'], conf['bundle_agg_dropouts'], conf["embedding_sizes"], conf["num_layerss"], conf["c_lambdas"], conf["c_temps"]):
        ...
        if conf["aug_type"] == "OP":
            assert item_level_dropout == 0 and bundle_level_dropout == 0 and bundle_agg_dropout == 0

        settings += ["Neg:%d" %(conf["neg_num"]), str(conf["batch_size_train"]), str(lr), str(l2_reg), str(embedding_size)] 

        conf["item_level_dropout"] = item_level_dropout
        conf["bundle_level_dropout"] = bundle_level_dropout
        conf["bundle_agg_dropout"] = bundle_agg_dropout
        conf["num_layers"] = num_layers
        settings += [str(item_level_dropout), str(bundle_level_dropout), str(bundle_agg_dropout), str(num_layers)]
        ...
        # model
        if conf['model'] == 'SelfCBR':
            model = SelfCBR(conf, dataset.graphs).to(device)
        else:
